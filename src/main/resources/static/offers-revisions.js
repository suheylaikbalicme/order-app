...
(async function init() {
  try {
    ...
    const r = await fetch(`/api/offers/${offerId}/revisions`);
    let rows = [];
    if (!r.ok) {
    ...
    function render() {
      ...
      if (btn && hasSnap) btn.addEventListener('click', () => openSnapshot(`Teklif #${offerId} - Rev.${x.revisionNo ?? ""}`, snap));
      });
    }
