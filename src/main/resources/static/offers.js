...
function addLineFromInputs() {
  ...
  offerCart.push({ itemCode, itemName, quantity, unitPrice, discountRate, vatRate });
  setMsg("");
  renderTable();
  calcTotals();

  // UX: add sonrası inputları resetle
  $("itemSelect").value = "";
  $("qty").value = "1";
  $("unitPrice").value = "0";
...
function renderTable() {
  ...
  offerCart.forEach((it, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td class="right"><span class="badge">${money(lineNet(it))}</span></td>
    ...
    const recalc = () => {
      ...
      tr.querySelector("span").textContent = money(lineNet(it));
      calcTotals();
    };
...
function calcTotals() {
  const t = computeTotals(offerCart);
...
async function saveOffer(action) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Para birimi seçilince otomatik kur çek
  ...
  $("saveDraftBtn").addEventListener("click", () => saveOffer("draft"));
  $("submitBtn").addEventListener("click", () => saveOffer("submit"));
